...
export function ContinueLearning({ sessions, onContinue }: ContinueLearningProps) {
  ...
  return (
    <section className="mb-8">
      <h3 className="text-headline-md font-bold mb-4">Lanjutkan Pelajaran</h3>
      <div className="flex gap-4 overflow-x-auto scroll-hide -mx-5 px-5 py-2 md:grid md:grid-cols-2 lg:grid-cols-3 md:overflow-visible md:mx-0 md:px-0">
        {sessions.map((session, idx) => {
          ...
          return (
            <button
              ...
              className={`min-w-[200px] md:min-w-0 bg-gradient-to-br ${gradient} rounded-3xl p-5 card-shadow flex flex-col gap-3 relative overflow-hidden active:scale-95 transition-all duration-200 border border-white/50 text-left shrink-0`}
            >
              ...
              <div className="relative z-10">
                <h4 className="font-bold text-label-lg leading-tight mb-0.5 line-clamp-1">
                  {session.chapterTitle}
                </h4>
                <p className="text-xs font-medium opacity-70">{session.subjectName}</p>
              </div>
              <div className="mt-auto relative z-10">
                <div className="h-2 w-full bg-white/40 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-ink-navy/60 rounded-full transition-all"
                    style={{ width: `${session.progressPercentage}%` }}
                  />
                </div>
              </div>
              <div className="absolute -bottom-2 -right-4 w-20 h-20 opacity-90 pointer-events-none">
                <img
                  alt=""
                  className="w-full h-full object-contain"
                  src={mascotUrl}
